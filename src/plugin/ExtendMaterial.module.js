...
//This script has been modified a little bit to work with the zappar webpack settings(?), so it's slightly different from the link above
...
function usUniformPairs( instance, uniforms ) {
...
function usUniforms( name, object, uniforms ) {
 
	usUniformPairs( object.uniforms, uniforms );
...
function applyConstants( name, uniform, defines, object, instance ) {
	...
	if ( flag !== undefined && ( flag.not === undefined || flag.not !== uniform.value ) )
...
function mapUniforms( name, uniforms, object ) {
	...
	if ( object.explicit === false ) {
		...
		return usUniforms( name, object, uniforms ); // cloneUniforms( THREE.ShaderLib[ name ].uniforms, uniforms, true );
		//return cloneUniforms( THREE.ShaderLib[ name ].uniforms, uniforms, true );
	}
